...
public sealed partial class LocalityToggleContext()
{
    ...
    internal void ControlPanelOpened(bool realMachine = false)
    {
        ...
        if (realMachine && ModEntry.Config.PerMachineControlPanel) Locality = PanelLocality.PerMachine;
        else if (ModEntry.Config.DefaultIsGlobal) Locality = PanelLocality.Global;
        else Locality = PanelLocality.PerLocation;
    ...
    public void ToggleLocality()
    {
        ...
        if (realMachine)
        {
            if (ModEntry.Config.DefaultIsGlobal) Locality = Locality switch
            {
    ...
    private static MsdKey makeDataKeyFor(PanelLocality locality, Item machine) => locality switch
    {
